...
We need to complete a given number of missions, 
where each mission requires going from Alpha to Beta and back to Alpha.

Calculate the time unit when all missions are completed, 
assuming we take the earliest available shuttle.
...
def solution(alpha2beta, beta2alpha, missions):
    ...
    curr_mission = 0
    max_time = 0
    while curr_mission < missions:
        curr_time = alpha2beta[curr_alpha] + 100
        while beta2alpha[curr_beta] < curr_time:
            curr_beta += 1
        max_time = max(max_time, beta2alpha[curr_beta] + 100)
        curr_beta += 1
        curr_alpha += 1
        curr_mission += 1
    return max_time
